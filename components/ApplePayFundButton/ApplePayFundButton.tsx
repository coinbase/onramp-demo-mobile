...
export default function ApplePayFundButton() {
  ...
  useEffect(() => {
    // Make a request to create an order when the component is mounted.
     console.log('FUNCTION CALLED: createOrder')
      createOrder();
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      console.log('Creating order with the following parameters: ', {
      ...
      if (response.paymentLink) {
        console.log('Setting payment link: ', response.paymentLink.url);
